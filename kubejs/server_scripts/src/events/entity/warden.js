EntityEvents.beforeHurt(event => {
    ...
    let inputBlock = "sculk_replaceable";
    let resultBlock = "minecraft:sculk";
    ...
    if (source.getType() == "sonic_boom") {
        ...
        // 计算直线的长度（最大差值）
        let dx = Math.abs(targetBlockPos.x - wardenBlockPos.x);
        let dy = Math.abs(targetBlockPos.y - wardenBlockPos.y);
        let dz = Math.abs(targetBlockPos.z - wardenBlockPos.z);
        let maxDelta = Math.max(dx, dy, dz);
